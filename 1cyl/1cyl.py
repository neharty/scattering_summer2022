...
parser = argparse.ArgumentParser(description='generate scattering plot for a given radius')
parser.add_argument('radius', metavar='a', type=float,
...
def u_radial(r, theta, bc):
    ...
    for i in range(r.shape[0]):
        for j in range(r.shape[1]):
            if mask[i,j]:
                sum_nums[i,j] = M_sum(r[i,j], k, a, err, bc)
            else:
                continue

    M = int(np.amax(sum_nums))
...
ax[1,0].set_title('neumann sum num, err = ' + str(err))
...
ax[1,1].set_title('dirichlet sum num, err = ' + str(err))
...
ax[1,2].set_title('impedance sum number, err = ' + str(err))
